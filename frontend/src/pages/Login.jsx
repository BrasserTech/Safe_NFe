...
export function Login() {
  ...
  const developmentCredentials = import.meta.env.DEV
    ? { email: "admin@safe-nfe.local", password: "123456" }
    : { email: "", password: "" };
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-8 shadow-soft">
        ...
        <input name="email" className="mt-2 w-full rounded-md border border-slate-200 px-3 py-3 outline-ocean" type="email" placeholder="admin@safe-nfe.local" defaultValue={developmentCredentials.email} required autoComplete="username" />
        <label className="mt-4 block text-sm font-semibold text-slate-700">Senha</label>
        <input name="password" className="mt-2 w-full rounded-md border border-slate-200 px-3 py-3 outline-ocean" type="password" placeholder="Sua senha" defaultValue={developmentCredentials.password} required autoComplete="current-password" />
        {import.meta.env.DEV && <p className="mt-3 text-xs text-slate-500">Acesso local de desenvolvimento preenchido automaticamente.</p>}
